...
public class FindPropertyResDto {
	...
	public static FindPropertyResDto fromEntity(Property property) {
		List<TagResDto> tagDtos = property.getPropertyTagMaps() == null? Collections.emptyList():property.getPropertyTagMaps().stream()
			.map(propertyTagMap -> TagResDto.fromEntity(propertyTagMap.getTag()))
			.toList();
